...
%DIF DEL manuscript-first-submission/manuscript.tex   Mon Dec 11 02:28:21 2023
%DIF ADD manuscript/manuscript.tex                    Thu Feb 22 12:24:53 2024
...
\title{Human \DIFdelbegin \DIFdel{Judgement Forecasting }\DIFdelend \DIFaddbegin \DIFadd{judgement forecasting }\DIFaddend of COVID-19 in the UK}
...
Upon pressing a button, participants could see a preview of the evolution of cases implied by their current $R_t$ forecast. However, due to lack of development time, participants could not preview the death forecast implied by their current input for $R_t$ nor could they influence the estimated case fatality ratio or delay between reported cases and reported deaths. Figure \DIFdelbegin \DIFdel{\ref{fig:screenshot-rt} }\DIFdelend \DIFaddbegin \DIFadd{4 in the Supplementary Information \mbox{%DIFAUXCMD
\cite{bosseSupplementaryInformationHuman2023} }\hskip0pt%DIFAUXCMD
}\DIFaddend shows a screenshot of the forecast interface for $R_t$ forecasts. 
...
\DIFdelbegin \DIFdel{Our analysis of the data largely follows the one outlined in \mbox{%DIFAUXCMD
\citet{bosseComparingHumanModelbased2022}}\hskip0pt%DIFAUXCMD
. }\DIFdelend We scored forecasts using the weighted interval score
\DIFdelbegin \DIFdel{(WIS, formulas given in the SI) 
\mbox{%DIFAUXCMD
\cite{bracherEvaluatingEpidemicForecasts2021}}\hskip0pt%DIFAUXCMD
. }\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{bracherEvaluatingEpidemicForecasts2021}}\hskip0pt%DIFAUXCMD
. For a single interval, the interval score is computed as 
  }$$\DIFadd{IS_\alpha(F,y) = (u-l) + \frac{2}{\alpha} \cdot (l-y) \cdot 1(y \leq l) + \frac{2}{\alpha} \cdot (y-u) \cdot 1(y \geq u), }$$ 
  \DIFadd{where $1()$ is the indicator function, $y$ is the true value, and $l$ and $u$ are the $\frac{\alpha}{2}$ and $1 - \frac{\alpha}{2}$ quantiles of the predictive distribution $F$, i.e., the lower and upper bound of a single prediction interval. For a set of $K$ prediction intervals and the median $m$, the score is computed as a weighted sum, 
  }$$\DIFadd{WIS = \frac{1}{K + 0.5} \cdot \left( w_0 \cdot |y - m| + \sum_{k = 1}^{K} w_k \cdot IS_{\alpha}(F, y) \right), }$$
  \DIFadd{where $w_k$ is a weight for every interval. Usually, $w_k = \frac{\alpha_k}{2}$ and $w_0 = 0.5$. 
}

\DIFaddend The WIS is a \DIFdelbegin \DIFdel{strictly }\DIFdelend proper scoring rule yielding non-negative values, with smaller values implying better performance. A forecaster, in expectation, optimises their score by providing a predictive distribution $F$ that is equal to the data-generating distribution $G$, and is therefore incentivised to report their true belief. The WIS can be understood as an approximation of the continuous ranked probability score (CRPS\DIFaddbegin \DIFadd{, \mbox{%DIFAUXCMD
\citealt{gneitingProbabilisticForecastsCalibration2007}}\hskip0pt%DIFAUXCMD
}\DIFaddend ) for forecasts in a quantile-based format\DIFdelbegin \DIFdel{, which itself }\DIFdelend \DIFaddbegin \DIFadd{. The CRPS, in turn, }\DIFaddend represents a generalisation of the absolute error to predictive distributions. The WIS can be decomposed into three separate penalty components \DIFaddbegin \DIFadd{(corresponding to the three terms in the definition of the interval score)}\DIFaddend : forecast dispersion (i.e.\DIFaddbegin \DIFadd{, }\DIFaddend uncertainty of forecasts), overprediction and underprediction. 
...
\DIFaddbegin \DIFadd{In the first part of the study period, most forecasting approaches (albeit not the direct crowd forecasts), showed a tendency to overpredict the increase in death numbers (see Figure \ref{fig:forecasts-scores}B). }\DIFaddend All forecasting approaches \DIFaddbegin \DIFadd{started to underpredict death numbers four weeks after the peak in case numbers on July 17, 2021, expecting a consequent drop in deaths that did not occur.
}
...
\subsection*{Experts and \DIFdelbegin \DIFdel{Non-Experts}\DIFdelend \DIFaddbegin \DIFadd{non-experts}\DIFaddend }
...
\caption{\bf{Predictive performance of self-reported "experts" and "non-experts" across forecast horizons.} \DIFaddFL{Forecasts from "experts" and "non-experts" were combined to two separate median ensembles, including both direct and $R_t$ forecasts. A-D: WIS stratified by forecast horizon for cases and deaths on the natural and log scale. E, F: Empirical coverage of the 50\% and 90\% prediction intervals stratified by forecast horizon and target type.}}
\label{fig:performance-experts}
\end{figure*}


\input{performance-table-experts}
...
During the study period, the case fatality ratio (CFR) \DIFaddbegin \DIFadd{likely }\DIFaddend changed quite quickly \DIFdelbegin \DIFdel{. This was related to the }\DIFdelend \DIFaddbegin \DIFadd{compared to the pre-study period. On the one hand, the }\DIFaddend rise of the Delta variant in the UK\DIFdelbegin \DIFdel{in the beginning of May with its higher case fatality rate \mbox{%DIFAUXCMD
\citep{bastIncreasedRiskHospitalisation2021}}\hskip0pt%DIFAUXCMD
. }\DIFdelend \DIFaddbegin \DIFadd{, which was first detected in the UK in March 2021, was estimated to have a higher CFR than previous variants \mbox{%DIFAUXCMD
\citep{bastIncreasedRiskHospitalisation2021, twohigHospitalAdmissionEmergency2022} }\hskip0pt%DIFAUXCMD
(although \mbox{%DIFAUXCMD
\citep{perez-guzmanEpidemiologicalDriversTransmissibility2023} }\hskip0pt%DIFAUXCMD
estimated it to be lower than that of the Alpha variant). On the other hand, the ongoing COVID-19 vaccination and growing natural immunity in the population had decreasing effects on the CFR. }\DIFaddend In addition, the age distribution of cases changed \DIFdelbegin \DIFdel{over the course of }\DIFdelend \DIFaddbegin \DIFadd{(hence modifying the overall CFR) throughout }\DIFaddend the study period in Summer 2021, in parts related to the European Football Championship \citep{dehningImpactEuro20202023}. \DIFaddbegin \DIFadd{Overall, the CFR was lower than during previous peaks of COVID-19 \mbox{%DIFAUXCMD
\citep{perez-guzmanEpidemiologicalDriversTransmissibility2023}}\hskip0pt%DIFAUXCMD
.
}\DIFaddend One possible hypothesis for the relatively good performance of human forecasts for deaths compared to previous studies might be that some models submitted to the Forecast Hub may have been more negatively affected by the changes in CFR during the study period than human forecasters or have been slower to update. \DIFaddbegin \DIFadd{The present study only saw a steady increase in death numbers, which one could argue is relatively easy to predict, making it difficult to compare forecast performance with performance in other settings. }\DIFaddend A confounding factor, when comparing results from this study and the one in Germany and Poland directly, is that we used a median ensemble to combine individual forecasts here, while the earlier study used a mean ensemble. 

\DIFaddbegin \DIFadd{Importantly, in this study our combined crowd ensemble ("epiforecasts-EpiExpert") contributed to the European Forecast Hub ensemble. This is in contrast to the study by \mbox{%DIFAUXCMD
\citet{bosseComparingHumanModelbased2022}}\hskip0pt%DIFAUXCMD
, where they compared crowd forecasts against a hypothetical ensemble excluding the crowd forecasts. In the original study, including the crowd forecasts improved the Hub ensemble on average (however, the overall number of models included in the German and Polish Hub ensemble was smaller than the number of models in the European Forecast Hub ensemble). In our study, comparisons between our crowd ensembles and the Forecast Hub ensemble are therefore confounded by the fact the combined crowd ensemble was included in the Forecast Hub ensemble, possibly leading us to underestimate differences between the two. 
}

\DIFaddend % \paragraph{Rt forecasting}

This study explored a novel method of forecasting infectious diseases that combines a human forecast of the estimated effective reproduction number $R_t$ with epidemiological modelling to map the $R_t$ forecast to a forecast of cases and deaths. One appeal of this approach is that the forecaster can directly forecast the generative process and how they believe it is affected by interventions and changes in behaviour. Computational modelling then takes care of dealing with details such as reporting delays, generation intervals, day of the week periodicity, and the relationship between different indicators. This could help reduce cognitive load, and make it easier to synthesise various sources in information into a single forecast, at least for forecasters who have an intuitive understanding of $R_t$. Though we note all of these modelling steps and the construction of the model itself requires the human constructing the model to make assumptions. Anecdotally, forecasters familiar to the authors reported high satisfaction with the forecasting experience. \DIFaddbegin \DIFadd{One important limitation of the approach is that $R_t$ values were estimated based on reported numbers of cases. This is susceptible to changes in testing and reporting and estimated $R_t$ values may not accurately reflect the true underlying transmission dynamics.
}\DIFaddend In our study, $R_t$ forecasts of cases were comparable to direct forecasts, with a tendency for good forecasts to improve when using the $R_t$ method and worse forecasts to deteriorate even more. Sample sizes, however, were very low. 
Given that forecasters could simulate cases in the app, it is also possible that forecasters were in \DIFdelbegin \DIFdel{reality }\DIFdelend \DIFaddbegin \DIFadd{fact }\DIFaddend directly forecasting cases. 
$R_t$ forecasts of deaths (which forecasters could not see in the app) were noticeably worse than direct forecasts of deaths. The computational model underlying our $R_t$ forecasts of deaths estimated a constant CFR and delay distribution using the last 4 weeks of data, therefore updating relatively slowly to new circumstances and was assumed to be constant over the four week forecast horizon. However, as mentioned before, the CFR \DIFaddbegin \DIFadd{likely }\DIFaddend evolved during the study period. Forecasters had no way of inspecting the death forecast implied by their $R_t$ forecast\DIFdelbegin \DIFdel{and }\DIFdelend \DIFaddbegin \DIFadd{, likely impacting predictive performance. They }\DIFaddend also had no way to adjust the CFR manually\DIFdelbegin \DIFdel{, likely impacting forecast accuracy}\DIFdelend . Giving human forecasters \DIFaddbegin \DIFadd{to see their implied death forecasts, as well as }\DIFaddend the ability to adjust the CFR and other model parameters would have increased complexity of the interface, but would have solved issues with the assumptions of the underlying model. Alternatively, a more complex model could have been used which allowed for time-varying CFR estimates and forecast these changes over the forecast horizon though this approach may still have struggled to cope with the rapid changes observed during the study period. Another important limitation is that we didn't have full sample trajectories of the $R_t$-values predicted by forecasters. Rather, trajectories had to be constructed based on the distributions provided for the different forecast horizons, which likely negatively affected forecasts. One potential way to disentangle the effect of the convolution model from the $R_t$ forecasts would have been to use the human forecasts for cases as an input to the second computational model, which could then have simulated deaths. Future work could expose forecasters to different combinations of these options with the aim of separating effects of the user interface from ones related to the structure of the underlying computational model.
...
\DIFadd{All data and code are available publicy under a MIT license under }\url{https://github.com/epiforecasts/uk-crowd-forecasting-challenge} \DIFadd{and }\url{https://doi.org/10.5281/zenodo.7897257}\DIFadd{. The data has been published separately here: }\url{https://doi.org/10.5281/zenodo.7897289}\DIFadd{. 
Supplementary Information \mbox{%DIFAUXCMD
\cite{bosseSupplementaryInformationHuman2023} }\hskip0pt%DIFAUXCMD
to this manuscript is available at }\url{https://doi.org/10.5281/zenodo.7897513}\DIFadd{. 
}\DIFaddend 


\DIFaddbegin \clearpage
\DIFaddend {\small\bibliographystyle{unsrtnat}
\bibliography{software, uk-forecasting-challenge}}

\DIFdelbegin %DIFDELCMD < \bigskip
%DIFDELCMD < \clearpage
%DIFDELCMD < %%%
\DIFdelend %DIF >  ================================= References =============================== %

%DIF <  =================================== Appendix =============================== %
%DIF >  \bigskip
%DIF >  \clearpage

%DIF <  Make sure that Appendix tables and Figures are separate
%DIF <  https://tex.stackexchange.com/questions/520193/is-it-possible-to-except-figures-in-the-appendix-from-being-repositioned-by-endf
%DIF < \processdelayedfloats
\DIFdelbegin %DIFDELCMD < \csname %%%
\DIFdel{efloat@restorefloats}%DIFDELCMD < \endcsname
%DIFDELCMD < %%%
...
%DIFAUXCMD
%DIFDELCMD < \renewcommand{\thefigure}{SI.\arabic{figure}}
%DIFDELCMD < \setcounter{figure}{0}
%DIFDELCMD < \renewcommand{\thetable}{SI.\arabic{table}} \setcounter{table}{0}
%DIFDELCMD < %%%
\DIFdelend %DIF >  % Make sure that Appendix tables and Figures are separate
%DIF >  % https://tex.stackexchange.com/questions/520193/is-it-possible-to-except-figures-in-the-appendix-from-being-repositioned-by-endf
%DIF >  %\processdelayedfloats
%DIF >  \csname efloat@restorefloats\endcsname

\DIFdelbegin \subsection*{\DIFdel{Weighted interval score}}
%DIFAUXCMD
%DIFDELCMD < \label{sec:wis}
%DIFDELCMD < %%%
\DIFdelend %DIF >  \appendix
%DIF >  \section*{Supplementary information}
%DIF >  \renewcommand{\thefigure}{SI.\arabic{figure}}
%DIF >  \setcounter{figure}{0}
%DIF >  \renewcommand{\thetable}{SI.\arabic{table}} \setcounter{table}{0}
...
\DIFdel{where $1()$ is the indicator function, $y$ is the true value, and $l$ and $u$ are the $\frac{\alpha}{2}$ and $1 - \frac{\alpha}{2}$ quantiles of the predictive distribution $F$, i.e. the lower and upper bound of a single prediction interval. For a set of $K$ prediction intervals and the median $m$, the score is computed as a weighted sum, 
  }\begin{displaymath}\DIFdel{WIS = \frac{1}{K + 0.5} \cdot \left( w_0 \cdot |y - m| + \sum_{k = 1}^{K} w_k \cdot IS_{\alpha}(F, y) \right), }\end{displaymath}%DIFAUXCMD
%DIFDELCMD <   %%%
\DIFdel{where $w_k$ is a weight for every interval. Usually, $w_k = \frac{\alpha_k}{2}$ and $w_0 = 0.5$. 
}\DIFdelend %DIF >  \subsection*{Weighted interval score}
%DIF >  \label{sec:wis}

\DIFdelbegin %DIFDELCMD < \input %%%
\DIFdel{SI/epinow2.tex
}\DIFdelend %DIF >  The weighted interval score (smaller values are better) is a proper scoring rule for quantile forecasts. It converges to the continuous ranked probability score (which itself is a generalisation of the absolute error to probabilistic forecasts) for an increasing number of intervals. The score can be decomposed into a dispersion (uncertainty) component and penalties for over- and underprediction. For a single interval, the score is computed as 
%DIF >    $$IS_\alpha(F,y) = (u-l) + \frac{2}{\alpha} \cdot (l-y) \cdot 1(y \leq l) + \frac{2}{\alpha} \cdot (y-u) \cdot 1(y \geq u), $$ 
%DIF >    where $1()$ is the indicator function, $y$ is the true value, and $l$ and $u$ are the $\frac{\alpha}{2}$ and $1 - \frac{\alpha}{2}$ quantiles of the predictive distribution $F$, i.e., the lower and upper bound of a single prediction interval. For a set of $K$ prediction intervals and the median $m$, the score is computed as a weighted sum, 
%DIF >    $$WIS = \frac{1}{K + 0.5} \cdot \left( w_0 \cdot |y - m| + \sum_{k = 1}^{K} w_k \cdot IS_{\alpha}(F, y) \right), $$
%DIF >    where $w_k$ is a weight for every interval. Usually, $w_k = \frac{\alpha_k}{2}$ and $w_0 = 0.5$. 

%DIF < table 4 weeks
%DIF >  \input SI/epinow2.tex

\DIFdelbegin %DIFDELCMD < \begin{figure*}
%DIFDELCMD < \centering
%DIFDELCMD < \includegraphics[width=0.99\textwidth]{../output/figures/num-forecasters.png}
%DIFDELCMD < %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
%DIFDELCMD < \bf{Number of forecasts across the study period.} %%%
...
%DIFDELCMD < %%%
\DIFdelend %DIF >  %table 4 weeks

\DIFdelbegin %DIFDELCMD < \begin{figure*}
%DIFDELCMD < \centering
%DIFDELCMD < \includegraphics[width=0.99\textwidth]{../output/figures/plot-data.png}
%DIFDELCMD < %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
%DIFDELCMD < \bf{Observed cases and deaths of COVID-19 in the UK}%%%
...
%DIF >  \centering
%DIF >  \includegraphics[width=0.99\textwidth]{../output/figures/plot-data.png}
%DIF >  \caption{\bf{Observed cases and deaths of COVID-19 in the UK}. A: Observed daily (bars) and weekly (black lines and points) numbers of cases and deaths as available through the European Forecast Hub when the study concluded in 2021. Daily numbers were multiplied by seven in order to appear on the same scale as weekly numbers. Red dots represent days for which the original data and the revised data disagreed by more than five percent. B: Revised data available as of February 14 2023. In August, Johns Hopkins University that provided the data switched the data stream for their death forecasts to reflect the number of death certificates that mentioned COVID-19 rather than the number of people who died within 28 days of a positive test. C: Difference between the original and revised weekly death numbers.}
%DIF >  \label{fig:plot-data}
%DIF >  \end{figure*}


\DIFdelbegin %DIFDELCMD < \begin{figure*}
%DIFDELCMD < \centering
%DIFDELCMD < \includegraphics[width=0.99\textwidth]{../output/figures/screenshot-crowd-classical.png}
%DIFDELCMD < %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
%DIFDELCMD < \bf{Screenshot of the direct forecasting interface.}%%%
}
%DIFAUXCMD
%DIFDELCMD < \label{fig:screenshot-classical}
%DIFDELCMD < \end{figure*}
%DIFDELCMD < %%%
\DIFdelend %DIF >  \input performance-table-horizon-4.tex



\DIFdelbegin %DIFDELCMD < \begin{figure*}
%DIFDELCMD < \centering
%DIFDELCMD < \includegraphics[width=0.99\textwidth]{../output/figures/screenshot-crowd-rt-app.png}
%DIFDELCMD < %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
%DIFDELCMD < \bf{Screenshot of the $R_t$ forecasting interface.}%%%
}
%DIFAUXCMD
%DIFDELCMD < \label{fig:screenshot-rt}
%DIFDELCMD < \end{figure*}
%DIFDELCMD < %%%
\DIFdelend %DIF >  \begin{figure*}
%DIF >  \centering
%DIF >  \includegraphics[width=0.99\textwidth]{../output/figures/screenshot-crowd-classical.png}
%DIF >  \caption{\bf{Screenshot of the direct forecasting interface.}}
%DIF >  \label{fig:screenshot-classical}
%DIF >  \end{figure*}


\DIFdelbegin %DIFDELCMD < \begin{figure*}
%DIFDELCMD < \centering
%DIFDELCMD < \includegraphics[width=0.99\textwidth]{../output/figures/performance-ranks-4.png}
%DIFDELCMD < %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
%DIFDELCMD < \bf{Ranks for all forecasting approaches for four week ahead forecasts}%%%
\DIFdelFL{. Colours indicate how often (out of 13 forecasts) a given approach got 1st, 2nd, 3rd, or 4th rank.}}
%DIFAUXCMD
%DIFDELCMD < \label{fig:performance-ranks-4}
%DIFDELCMD < \end{figure*}
%DIFDELCMD < %%%
\DIFdelend %DIF >  \begin{figure*}
%DIF >  \centering
%DIF >  \includegraphics[width=0.99\textwidth]{../output/figures/screenshot-crowd-rt-app.png}
%DIF >  \caption{\bf{Screenshot of the $R_t$ forecasting interface.}}
%DIF >  \label{fig:screenshot-rt}
%DIF >  \end{figure*}
...
\DIFdelend %DIF >  \begin{figure*}
%DIF >  \centering
%DIF >  \includegraphics[width=0.99\textwidth]{../output/figures/performance-ranks-4.png}
%DIF >  \caption{\bf{Ranks for all forecasting approaches for four week ahead forecasts}. Colours indicate how often (out of 13 forecasts) a given approach got 1st, 2nd, 3rd, or 4th rank.}
%DIF >  \label{fig:performance-ranks-4}
%DIF >  \end{figure*}


\DIFdelbegin %DIFDELCMD < \begin{figure*}
%DIFDELCMD < \centering
%DIFDELCMD < \includegraphics[width=0.99\textwidth]{../output/figures/performance-expert.png}
%DIFDELCMD < %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
%DIFDELCMD < \bf{Predictive performance of self-reported "experts" and "non-experts" across forecast horizons.} %%%
...
%DIFAUXCMD
%DIFDELCMD < \label{fig:performance-experts}
%DIFDELCMD < \end{figure*}
%DIFDELCMD < %%%
\DIFdelend %DIF >  \clearpage


\DIFdelbegin %DIFDELCMD < \input{manuscript/performance-table-experts}
%DIFDELCMD < %%%
\DIFdelend %DIF >  \begin{figure*}
%DIF >  \centering
%DIF >  \includegraphics[width=0.99\textwidth]{../output/figures/performance-expert.png}
%DIF >  \caption{\bf{Predictive performance of self-reported "experts" and "non-experts" across forecast horizons.} Forecasts from "experts" and "non-experts" were combined to two separate median ensembles, including both direct and $R_t$ forecasts. A-D: WIS stratified by forecast horizon for cases and deaths on the natural and log scale. E, F: Empirical coverage of the 50\% and 90\% prediction intervals stratified by forecast horizon and target type.}
%DIF >  \label{fig:performance-experts-SI}
%DIF >  \end{figure*}


\DIFdelbegin %DIFDELCMD < \input{manuscript/performance-table-experts-4}
%DIFDELCMD < %%%
\DIFdelend %DIF >  %\input{manuscript/performance-table-experts}

%DIF <  See this guide for more information on BibTeX:
%DIF <  http://libguides.mit.edu/content.php?pid=55482&sid=406343
%DIF >  \input{manuscript/performance-table-experts-4}


%DIF <  For more author guidance please see:
%DIF <  http://wellcomeopenresearch.org/for-authors/article-guidelines
%DIF >  % See this guide for more information on BibTeX:
%DIF >  % http://libguides.mit.edu/content.php?pid=55482&sid=406343

%DIF <  When all authors are happy with the paper, use the 
%DIF <  ‘Submit to WELLCOME OPEN RESEARCH' button from the menu above
%DIF <  to submit directly to the open life science journal Wellcome Open Research.
%DIF >  % For more author guidance please see:
%DIF >  % http://wellcomeopenresearch.org/for-authors/article-guidelines
...
%DIF <  We hope you find the Wellcome Open Research Overleaf template useful,
